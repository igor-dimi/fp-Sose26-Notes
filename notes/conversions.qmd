...
constructed. But more explicitly a constructor `CPFloat(const FP &)` can be implemented too. 
...
In C++, we can check at compile time whether an object of type `T_out` can be constructed from an argument type
...
In both cases `T_in` is an `FP` type.  as we've seen in @lst-get-d-method the `double` representation of an
`FP` number `x` can be extracted with:
...
Here, the helper trait `is_hdnum_fp_v<T>` detects whether a type `T` is one of HDNUM's GMP-based high
precision numbe types `hdnum::FP<m>`. This is a compile-time Boolean value, and analogous to the standard
library traits such as `std::is_constructible_v`.
...
3) The last part
   ...
   makes the trait insensitive to references and `const` / `volatile` qualifiers. For example
   this way all of the following are treated as the same:
   ...
   Without removing references and cv-qualifiers, the specialization would not match types such as
   `hdnum::FP<m>&`
...
The `class...` syntax denotes a template parameter pack and means that `always_false` can accept any number
of template type arguments. In our case we use two arguments:
...
It is important that `always_false<T_out, T_in>::value` depends on template parameters `T_out` and `T_in`, as it
lets us create a compile-time `false` value that is evaluated when the templates are instantiated and not when
the function is parsed. Omitting and writing simply `static_assert(false, ...)` would cause the assertion to be
checked when the function template is parsed, and not only when the template is instantiated and the final branch
is actually selected for the concrete instantiation. 
...
## Testing `hdnum_conversions.hpp`

The file `test_conversion.cc` tets the conversion infrastructure implemented in
`hdnum_conversions.hpp`. It's goals are to test that:

1. `scalar_cast<T_out>(x)` compiles for representative type.
2. `scalar_cast<T_out>(x)` returns the requested output type.
3. lossy low-precision conversions visibly loose information.
4. `convert(Vector)` applies `scalar_cast` element-wise correctly.
5. `convert(DenseMatrix)` applies `scalar_cast` elementwise correctly.
6. Vector and Matrix sizes are handled correctly.
7. Wrong-sized output contaniers throw exceptions
8. The convenience functions`convert_vector` and `convert_matrix` allocate correctly.
...
```cpp
mpir::scalar_cast<T_out>(x)
```

can be instantiated for representative input/output type paairs. For each pair,
several sample values are tested:
...
The result is checked at compile time with

```cpp
static_assert(std::is_same_v<decltype(y), T_out>);
```

To verify that `scalar_cast<T_out>(x)` returns the requested output type `T_out`.
...
It is verified that conversion from `double` to `CPFloat` indeed looses precision
using a value such as `1.23456789012345`.

This is only a useful sanity check that the conversion is really taking place.

### Lossless Conversion Tests

It is also verified that conversion from a lower precision to a higher precision
do not indeed loose information.
...
for a representative vector of length 5 and checks that each component of the output
agrees with applying `scalar_cast<T_out>` to the corresponding input component.

The comparison is performed approximately by conveting both values to `double` and using
a small relative tolerance with the function
...
```cpp
mpir::convert_vector<T_out>(in)
```

is tested, by verifying the resulting type and converted values.
...
on a representative `5 x 5` matrix, by looping over the output values and comparing them against the result of applying `scalar_cast` to corresponding input entries.
...
1. Native $\leftrightarrow$ Native: 
   * `FP32 <-> FP64`
2. Working precision $u \rightarrow$ low precision $u_f$: 
   * `FP64 -> FP16 / bfloat16 / FP8`
3. Low Precision $u_f \rightarrow$ working precision $u$:
   * `FP16 / bfloat16 / FP8 -> FP64`
4. Working precision $u \rightarrow$ residual precision $u_r$
   * `FP64 -> FP128 / FP256`
5. Residual precision $u_r \rightarrow$ working precision $u$
   * `FP128 / FP256 - > FP64 `
6. Low precision $u_f \leftrightarrow$ high residual/reference precision $u_r$
   * `FP128 / FP256 <-> FP16 / bfloat16 / FP8`
7. High precision $\leftrightarrow$ high precision
   * `FP128 <-> FP256`
...
The tests also include low to high-precision conversions not required directly
by the algorithm but useful nevertheless.
